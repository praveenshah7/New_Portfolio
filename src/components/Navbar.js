    ...
    <>
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.3rem 5rem', background: scrolled ? 'rgba(5,5,7,0.92)' : 'transparent', backdropFilter: scrolled ? 'blur(20px)' : 'none', borderBottom: scrolled ? '1px solid rgba(255,255,255,0.07)' : '1px solid transparent', transition: 'all 0.4s ease' }}>
          ...
          {links.map(link => (
            <li key={link}>
              <a href={`#${link.toLowerCase()}`}
                style={{ color: active === link.toLowerCase() ? '#00ff88' : '#6b6b80', fontFamily: "'Space Mono', monospace", fontSize: '.7rem', letterSpacing: '2px', textTransform: 'uppercase', textDecoration: 'none', transition: 'color 0.2s' }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '.5rem', fontFamily: "'Space Mono', monospace", fontSize: '.65rem', color: '#00ff88' }} className="nav-status">
          ...
          <button onClick={() => setMenuOpen(!menuOpen)}
            style={{ display: 'none', background: 'none', border: '1px solid rgba(255,255,255,0.14)', padding: '.4rem .6rem', cursor: 'pointer', color: '#e8e8f0', fontSize: '1rem' }}
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div style={{ position: 'fixed', top: '60px', left: 0, right: 0, zIndex: 99, background: 'rgba(5,5,7,0.98)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.07)', padding: '1.5rem' }}>
          {links.map(link => (
            <a key={link} href={`#${link.toLowerCase()}`}
              onClick={() => setMenuOpen(false)}
              style={{ display: 'block', padding: '.8rem 0', color: active === link.toLowerCase() ? '#00ff88' : '#6b6b80', fontFamily: "'Space Mono', monospace", fontSize: '.8rem', letterSpacing: '2px', textTransform: 'uppercase', textDecoration: 'none', borderBottom: '1px solid rgba(255,255,255,0.04)' }}
            >{link}</a>
          ))}
        </div>
      )}
