...
export default function About() {
  ...
  useEffect(() => {
    const obs = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting && !hasStarted.current) {
        ...
        terminalLines.forEach((_, i) => setTimeout(() => setVisibleLines(i + 1), i * 600));
      }
    }, { threshold: 0.3 });
  ...
  return (
    <section id="about" ref={sectionRef} style={{ padding: '7rem 5rem', background: '#0d0d12' }}>
      <ScrollReveal>
        <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '.65rem', letterSpacing: '3px', color: '#00ff88', textTransform: 'uppercase', marginBottom: '.5rem' }}>Section 01</div>
      ...
      <div className="about-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'start' }}>
          ...
          <div style={{ background: '#0a0a10', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', overflow: 'hidden' }}>
            ...
            <div style={{ background: '#13131a', padding: '.7rem 1rem', display: 'flex', alignItems: 'center', gap: '.6rem', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
              {['#ff5f57','#febc2e','#28c840'].map(c => (
                <span key={c} style={{ width: '10px', height: '10px', borderRadius: '50%', background: c, display: 'inline-block', flexShrink: 0 }} />
              ))}
              <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '.62rem', color: '#6b6b80', marginLeft: '.5rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>praveen@portfolio ~</span>
            </div>

            {/* Terminal body */}
            <div style={{ padding: '1.2rem', fontFamily: "'Space Mono', monospace", fontSize: '.75rem', lineHeight: 1.8, minHeight: '240px', overflowX: 'hidden', wordBreak: 'break-word' }}>
              {terminalLines.slice(0, visibleLines).map((line, i) => (
                <div key={i} style={{ marginBottom: '.7rem' }}>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.3rem' }}>
                    <span style={{ color: '#00ff88', flexShrink: 0 }}>➜</span>
                    <span style={{ color: '#7c3aed', flexShrink: 0 }}>~/portfolio</span>
                    <span style={{ color: '#e8e8f0' }}>{line.cmd}</span>
                  </div>
                  <div style={{ color: '#a0a0b8', paddingLeft: '.8rem', whiteSpace: 'pre-line', wordBreak: 'break-word', fontSize: '.72rem' }}>{line.out}</div>
                </div>
              ))}
              {visibleLines < terminalLines.length && (
                <div style={{ display: 'flex', gap: '.3rem' }}>
                  ...
                  <span style={{ display: 'inline-block', width: '8px', height: '1em', background: '#00ff88', animation: 'blink 1s infinite', verticalAlign: 'middle' }} />
                </div>
              )}
            </div>
          </div>
        ...
        <ScrollReveal delay={200} direction="right">
          <div>
            <p style={{ color: '#6b6b80', lineHeight: 1.85, fontSize: '.95rem', marginBottom: '1.2rem' }}>
              I'm <strong style={{ color: '#e8e8f0', fontWeight: 500 }}>Praveen Shah</strong> — a developer who believes great software lives at the intersection of clean code and thoughtful design.
            </p>
            <p style={{ color: '#6b6b80', lineHeight: 1.85, fontSize: '.95rem', marginBottom: '1.2rem' }}>
              Currently at <strong style={{ color: '#e8e8f0', fontWeight: 500 }}>Supplyvalid Pvt. Ltd.</strong> building production React apps. Previously shipped <strong style={{ color: '#e8e8f0', fontWeight: 500 }}>JoshGuru</strong> education platform at Trihari Smart Solutions.
            </p>
            <p style={{ color: '#6b6b80', lineHeight: 1.85, fontSize: '.95rem', marginBottom: '2rem' }}>
              I care deeply about <strong style={{ color: '#e8e8f0', fontWeight: 500 }}>performance, pixel precision, and great UX</strong>. Always learning — currently deep in React ecosystem and ML.
            </p>
